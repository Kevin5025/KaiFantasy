...
public class HealthRingMeter : Meter {
	...
	private void updateHealthRingMasks(float totalCapacity) {
		...
		for (int m = 0; m < maskArray_.Length; m++) {
			...
			maskArray_[m].transform.localEulerAngles = new Vector3(0, 0, 360f * cumulativeCapacity / totalCapacity);
			maskArray_[m].fillAmount = healthStateFill / totalCapacity;
	...
	private void updateHealthRingTicks(float totalCapacity) {
		...
		for (int t = 0; t < numTicks; t++) {
			if (healthRingTickList.Count <= t) {
				healthRingTickList.Add(Instantiate(WorldCanvasManager.worldCanvasManager_.ringThinTickPrefab_));
